...
FromSignature : Signature path -> List (n ** p : PathComp n ** (Args p -> Ret p))
FromSignature ps {path} = signatureHelp ps
  where
    signatureHelp : PathList fnPath -> List (n ** p : PathComp n ** (Args p -> Ret p))
...
stringSig n p parser printer handler =
    map (printer . handler) . (checkLength n >=> parser)
  where
...
handleAllPaths : PathList path -> List (List String -> Maybe String)
handleAllPaths [] {path = []} = []
...
||| Given an implementation of a path, return a list of function for each possible route
forAllPaths : Signature path -> List (List String -> Maybe String)
